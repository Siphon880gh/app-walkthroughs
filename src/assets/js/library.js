...
function renderUrlList(screens, project) {
    return `<div class="url-list">${screens.map(screen => {
        ...
        const picked = selectedUrlIds.has(screen.id);
        return `<div class="url-row ${project.activeScreenshotId === screen.id ? 'selected' : ''}${picked ? ' picked' : ''}"><img class="url-thumb" src="${safeImage(screen.dataUrl)}" alt=""><div class="url-copy"><button type="button" class="url-name" data-action="select-screen" data-id="${esc(screen.id)}">${esc(screen.name)}</button><div class="url-value">${url ? esc(url) : 'No hosted URL'}</div></div><div class="url-actions"><button type="button" class="button small" data-action="copy-screen-url" data-url="${esc(url)}" ${url ? '' : 'disabled'}>Copy</button><span class="url-actions-sep" aria-hidden="true"></span><input class="url-check" type="checkbox" data-action="toggle-url-select" data-id="${esc(screen.id)}" aria-label="Select ${esc(screen.name)}" ${picked ? 'checked' : ''} ${url ? '' : 'disabled'}></div></div>`;
    }).join('')}</div>`;
}
...
function renderLibrary() {
    ...
    const selectedUrls = screens.filter(screen => selectedUrlIds.has(screen.id) && screenUrl(screen.dataUrl));
    const hostedIds = screens.filter(screen => screenUrl(screen.dataUrl)).map(screen => screen.id);
    const allPicked = hostedIds.length > 0 && selectedUrls.length === hostedIds.length;
    const selectAll = libraryListMode && hostedIds.length ? `<label class="select-all"><input class="url-check" type="checkbox" data-action="select-all-urls" data-ids="${esc(hostedIds.join(' '))}" ${allPicked ? 'checked' : ''}>Select all</label>` : '';
    const copySelected = (libraryListMode && selectedUrls.length ? `<button type="button" class="button small" data-action="copy-selected-urls">Copy selected <span class="count-pill">${selectedUrls.length}</span></button>` : '') + selectAll;
