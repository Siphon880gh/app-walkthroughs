...
function renderUrlList(screens, project) {
    return `<div class="url-list">${screens.map(screen => {
        ...
        return `<div class="url-row ${project.activeScreenshotId === screen.id ? 'selected' : ''}"><img class="url-thumb" src="${safeImage(screen.dataUrl)}" alt=""><div class="url-copy"><button type="button" class="url-name" data-action="select-screen" data-id="${esc(screen.id)}">${esc(screen.name)}</button><div class="url-value">${url ? esc(url) : 'No hosted URL'}</div></div><button type="button" class="button small" data-action="copy-screen-url" data-url="${esc(url)}" ${url ? '' : 'disabled'}>Copy</button></div>`;
    }).join('')}</div>`;
}
