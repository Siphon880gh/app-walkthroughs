...
function selectedTextAnnotation(screen = activeScreen()) {
    ...
    const ann = (screen.annotations || []).find(item => item.id === selectedAnnotationId);
...
function renderEditor() {
    ...
    const annotationRows = annotations.map((ann, index) => {
        ...
        const nameMarkup = isText
            ? `<button type="button" class="annotation-pick" data-action="select-annotation" data-id="${esc(ann.id)}" data-focus="note">${index + 1}. ${esc(name)}</button>`
            : `<span class="annotation-name">${index + 1}. ${esc(name)}</span>`;
