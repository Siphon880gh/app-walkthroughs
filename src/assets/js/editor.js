...
function renderMarkEditor(ann) {
    ...
    const title = ann.type === 'callout-pin' ? 'PIN' : ann.type === 'highlight' ? 'HIGHLIGHT' : ann.type === 'spotlight' ? 'SPOTLIGHT' : ann.type === 'arrow' ? 'ARROW' : ann.type === 'circle' ? 'CIRCLE' : 'BOX';
...
function renderEditor() {
    ...
    const tools = [['select','Select'],['callout-pin','Pin'],['highlight','Highlight'],['rectangle','Box'],['circle','Circle'],['arrow','Arrow'],['text-box','Text'],['spotlight','Spotlight']];
    const toolIcon = value => value === 'select'
        ...
        : value === 'callout-pin' ? '●' : value === 'highlight' ? '▧' : value === 'rectangle' ? '□' : value === 'circle' ? '○' : value === 'arrow' ? '↗' : value === 'text-box' ? 'T' : '◉';
    ...
    const fitButtons = [['height','↕','Fit height'],['width','↔','Fit width']].map(([value,icon,label]) => `<button type="button" class="tool-button ${canvasFit === value ? 'active' : ''}" data-action="canvas-fit" data-fit="${value}" aria-pressed="${canvasFit === value ? 'true' : 'false'}" title="${label}">${icon}<span>${label}</span></button>`).join('');
    const fitClass = canvasFit === 'auto' ? '' : ` fit-${canvasFit}`;
    const fitToggle = `<button type="button" class="tool-button canvas-fit-toggle ${canvasFitOpen || canvasFit !== 'auto' ? 'active' : ''}" data-action="toggle-canvas-fit" aria-expanded="${canvasFitOpen ? 'true' : 'false'}" title="${canvasFitOpen ? 'Hide fit options' : 'Fit options'}" aria-label="Fit options"><svg class="tool-icon" viewBox="0 0 16 16" aria-hidden="true"><path fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" d="M2 6V2h4M10 2h4v4M14 10v4h-4M6 14H2v-4"/></svg></button>`;
    return `<section class="main-pane"><div class="studio-layout">${renderFilmstrip()}<div class="stage-shell"><div class="stage-toolbar"><div class="tool-group draw-tools">${toolButtons}</div><div class="tool-group">${historyButtons}</div><div class="tool-group">${colors}</div></div><div class="stage-area"><div class="stage${fitClass}"><div class="canvas-wrap ${screen.deviceFrame === 'desktop' ? 'desktop':'mobile'}${fitClass}${selectedTool === 'select' ? ' tool-select' : ''}" id="annotationCanvas" data-screen-id="${esc(screen.id)}"><img src="${safeImage(screen.dataUrl)}" alt="Annotating ${esc(screen.name)}" draggable="false"><div class="annotation-layer" id="annotationLayer">${annotationMarkup(annotations, true)}</div></div></div><div class="tool-group canvas-fit${canvasFitOpen ? ' open' : ''}" role="group" aria-label="Canvas fit">${canvasFitOpen ? fitButtons : ''}${fitToggle}</div></div><div class="stage-status"><span>${screen.width} × ${screen.height} · COORDINATES NORMALIZED 0–100%</span><span>${annotations.length} ANNOTATION${annotations.length === 1 ? '':'S'}</span></div></div><aside class="inspector"><div class="panel-head"><h2>Snapshot inspector</h2><span class="tag">${esc(screen.platform)}</span></div><div class="inspector-tabs"><button class="inspector-tab ${inspectorTab === 'analysis' ? 'active':''}" data-action="inspector-tab" data-tab="analysis">Analysis</button><button class="inspector-tab ${inspectorTab === 'layers' ? 'active':''}" data-action="inspector-tab" data-tab="layers">Layers (${annotations.length})</button></div><div class="panel-scroll" style="padding:0">${renderNoteEditor(selectedTextAnnotation(screen))}${renderMarkEditor(selectedAnnotation(screen))}${inspectorTab === 'analysis' ? `<div class="section"><label class="field"><span class="field-label">FILE NAME</span><input class="input" data-screen-name="${esc(screen.id)}" value="${esc(screen.name)}" maxlength="120"></label></div><div class="section"><button class="button primary" style="width:100%" data-action="save-snapshot">Save snapshot</button><button class="button" style="width:100%;margin-top:7px" data-action="save-snapshot-as">Save snapshot as</button><button class="button" style="width:100%;margin-top:7px" data-action="add-to-story" data-id="${esc(screen.id)}">Add to walkthrough</button></div>` : `<div class="section"><div class="section-title"><span>ANNOTATIONS</span><button class="button ghost small danger" data-action="clear-annotations" ${annotations.length ? '':'disabled'}>Clear all</button></div><div class="annotation-list">${annotationRows || '<p style="color:var(--dim);font-size:10px;line-height:1.5">Choose a tool and draw directly on the screen. Coordinates remain stable at any display size.</p>'}</div></div><div class="section"><div class="section-title">DRAWING HELP</div><p style="color:var(--dim);font-size:10px;line-height:1.6">Select is the cursor. Click a mark to change its color or size. Click a text note to edit its words. Drag a selected note to move it, and drag its corner to resize. Pins are placed with a click. Highlights, boxes, circles, arrows, and spotlights are drawn by dragging.</p></div>`}</div></aside></div></section>`;
}
