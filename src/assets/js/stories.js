...
function storyChooser(storyOptions, story) {
    return `<div class="story-chooser"><label class="story-select-field"><span>WALKTHROUGH</span><select class="select" data-action="change-story">${storyOptions}</select></label><label class="story-select-field"><span>CATEGORY</span><select class="select story-category-select ${storyCategoryTone(story)}" data-action="change-story-category">${storyCategoryOptions(story.category)}</select></label><div class="story-chooser-actions"><button class="button" data-action="new-story" title="New walkthrough">＋ New</button><button class="button" data-action="open-photo-picker">＋ Add photo</button></div></div>`;
}
